...
static LmHandlerCallbacks_t Callbacks = {
    .GetBatteryLevel = LoRaWAN_Platform_GetBatteryLevel,
    .GetTemperature  = LoRaWAN_Platform_GetTemperature,
    .GetUniqueId     = NULL,
    .GetDevAddr      = NULL,
    ...
    .OnBeaconStatusChange  = OnBeaconStatus,
...
bool LoRaWAN_SetJoinCredentials(const uint8_t joinEui[8], const uint8_t appKey[16]) {
    if (!joinEui || !appKey) return false;
...
void LoRaWAN_Init(const lorawan_otaa_keys_t* otaa) {
    ...
    // Initialize LmHandler with callbacks and firmware version
    LmHandlerInit(&Callbacks, APP_VERSION);
    
    // Configure LmHandler with parameters
    LmHandlerConfigure(&Params);

    // Set DevEUI via SecureElement
    ...
    // Start OTAA join procedure (activation type, forceRejoin)
...
int LoRaWAN_Send(const uint8_t* buf, uint8_t len, uint8_t fport, bool confirmed) {
    ...
    LmHandlerAppData_t appData = {
        .Buffer = (uint8_t*)buf,
        .BufferSize = len,
        .Port = (fport == 0) ? LORAWAN_DEFAULT_FPORT : fport
    };
    LmHandlerMsgTypes_t msgType = confirmed ? LORAMAC_HANDLER_CONFIRMED_MSG : LORAMAC_HANDLER_UNCONFIRMED_MSG;

    // LmHandlerSend requires allowDelayedTx parameter (set to true for duty cycle compliance)
    if (LmHandlerSend(&appData, msgType, true) == LORAMAC_HANDLER_SUCCESS) {
        return 0;
    }
    return -3;
}
...
static void OnJoinRequest(LmHandlerJoinParams_t* params) {
    ...
    if (params->Status == LORAMAC_HANDLER_SUCCESS) {
        ...
        // Retry join with OTAA and no force rejoin
        LmHandlerJoin(ACTIVATION_TYPE_OTAA, false);
    }
}
...
static void OnTxData(LmHandlerTxParams_t* params) {
    if (!params) return;
    APP_LOG(TS_ON, VLEVEL_L, "LoRaWAN: TX done DR%d\r\n", params->Datarate);
}

static void OnRxData(LmHandlerAppData_t* appData, LmHandlerRxParams_t* params) {
    if (!appData || !params) return;
    APP_LOG(TS_ON, VLEVEL_M, "LoRaWAN: RX port=%u len=%u RSSI=%d SNR=%d\r\n",
...
static void OnClassChange(DeviceClass_t newClass) {
    ...
    if (newClass == CLASS_B) {
        ...
        LmHandlerPingSlotReq(LORAWAN_CLASSB_PINGSLOT_PERIODICITY);
    }
#endif
}
