...
char *uart_req_w_ptr;
volatile uint8_t uart_data_ready = 0;	// 1 - data in uart_req_buf ready
static UART_HandleTypeDef *_curUart = NULL;		// aktualny uart
 ...
 */
const UTIL_ADV_TRACE_Driver_s UTIL_TraceDriver = { vcom_Init, vcom_DeInit, vcom_ReceiveInit, vcom_Trace_DMA, };
...
UTIL_ADV_TRACE_Status_t vcom_Trace_DMA(uint8_t *p_data, uint16_t size)
{
	...
	//return HAL_UART_Transmit(&huart1, p_data, size, 10000);	// MT 13.1.2026 - no DMA
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	...
	HAL_UART_RxCharMT(huart, charRx);
...
{
	UTIL_ADV_TRACE_Status_t status =  UTIL_ADV_TRACE_OK;//vcom_Trace_DMA((uint8_t*) strInfo, strlen(strInfo));
	vcom_Trace((uint8_t*) strInfo, strlen(strInfo));
	return status;
}
